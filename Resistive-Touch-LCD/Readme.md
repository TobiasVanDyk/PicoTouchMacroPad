...
Serial2Pico loads the current button definitions, labels, Layouts and Layers configuration from the TouchLCD when the [Open Port] and [Sync Pad-PC] buttons on the Comms tab of Serial2Pico are pressed. This needs a recent version of the TouchLCD firmware, but it can be used with older firmware but then with fewer functions. The current build PC-App-to-Pico-Firmware combination is: [**Serial2Pico75**](https://github.com/TobiasVanDyk/PicoTouchMacroPad/tree/main/Serial2PicoApp/Serial2Pico74.zip), and the RP2350B **Pico2A4LCD.zip** and the Pico 2 **VolumeMacroPad-Pico2-35IPS-339.zip** in their respective build folders Capacitive-Touch-LCD and Pico2. 

From Layouts 1, 3, and 4 (Macro Sets M, S and T), nine sets of 24 keys of types (b), (c), (d), and (e), (mM01-mM24, sS01-sS24, tT02-T24), where each keyset M, S, T can have descriptive and easily changeable, 5-character-maximum-length, labels, as well as another six sets of 99 files (aA01-aA99 and kK01-kK99), are available (note 1). Each of the Macro and text keys and files, can be further linked through a plaintext list of 3-letter filenames with additional processing such as delay and repeat, storage switching, and chaining  or further linking. There are also another 456 (19 sets of 24 files), text files which can be accessed from the SDCard using the same macro-keys. Processing of very large text files, tested up to 67kB (manual.h), but [**probably only limited by SDFS or LittleFS or FAT32 filesystem size constraints**](https://github.com/TobiasVanDyk/PicoTouchMacroPad/blob/main/Resistive-Touch-LCD/DoLargeFile.png), are available through the nKeys sets.<br> 
...
This means it is possible with a single macropad keypress to open for example photoshop on a PC, and at the same time re-configure the macropad keys for photoshop-specific functions. See Note 8 in the [**manual**](https://github.com/TobiasVanDyk/PicoTouchMacroPad/blob/main/Resistive-Touch-LCD/pico1-manual.md) for more detail.<br>

<img src="images/1million-macros.jpg" width="40" height="30"/> <img src="images/nKeys106.jpg" width="40" height="30"/> **Note 7:** The number of available macros are easily expanded to 1 million or even 10 million. Refer to [**Src-1M-macros**](https://github.com/TobiasVanDyk/PicoTouchMacroPad/tree/main/Resistive-Touch-LCD/Pico1/Src-1M-macros)). This has 1,079,568 possible nKeys, or 833 pages of 12 keys for each of the 36 first characters over the three different sets of files. All 9,996 nKeys per character are easily accessed via the Pad+ and Pad- PageUp and PageDown Pads.
