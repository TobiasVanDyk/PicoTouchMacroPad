...
Serial2Pico loads the current button definitions, labels, Layouts and Layers configuration from the TouchLCD when the [Open Port] and [Sync Pad-PC] buttons on the Comms tab of Serial2Pico are pressed. This needs a recent version of the TouchLCD firmware, but it can be used with older firmware but then with fewer functions. The current build PC-App-to-Pico-Firmware combination is: [**Serial2Pico75**](/Serial2PicoApp/Serial2Pico74.zip), and the RP2350B **Pico2A4LCD.zip** and the Pico 2 **VolumeMacroPad-Pico2-35IPS-339.zip** in their respective build folders Capacitive-Touch-LCD and Pico2. 
